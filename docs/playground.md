...
| Lease (in Fleet) | hand one VM one task: image, kind (`none`, `sfn`, `durable`, `http`, `sqs`, `eventbridge`), token, target, task JSON, budget, heartbeat timeout, slack, heartbeat interval; the job log shows the encoded `runHookPayload` and `clientToken`, the exact RunMicrovm request, then every phase and log line from inside the VM until the lease completes; with `shards` > 1 a task template (`{i}` = shard index), the plan sentence first, a refusal when the plan is rejected or the shards exceed the concurrency limit, then `lease_many` and the fleet job until every shard is done | `Lease`, `LeasePolicy`, `encode_payload`, `FleetManager.lease`, `FleetManager.plan`, `FleetManager.lease_many`, `EndpointClient.status` |
| Fleet job (in Fleet) | a toggle next to auto-refresh opens a table with one row per RUNNING member of an image (lease id, phase, progress, elapsed, state) and the footer `done D/N, running R, lost L, slowest`, polled every 2 s; a member that does not answer shows the reason in its row | `FleetMonitor.job_status` |
...
The "fleet job" switch in the Fleet view opens a card that calls `GET /api/fleet/jobs?image=<name>&port=<port>` every two seconds while it is open. The server runs `FleetMonitor.job_status(image)`: one `GET /status` per RUNNING member of the image, eight in parallel with a five second limit each, and returns `{"items": [{"microvm_id", "lease_id", "phase", "progress", "elapsed_s", "done", "lost", "error"}], "summary": {"total", "done", "running", "lost", "failed", "unanswered", "slowest"}}`; a member that cannot answer becomes `{"microvm_id", "error"}` without hiding the others. Clicking a row opens that VM's job panel. The same table is `mvm watch --image NAME` in the terminal. In dry run without AWS the card shows the sample fleet, each member at a different point of the sample job.

`POST /api/lease/run` with `shards` (an integer above 1) and an optional `task_template` (a JSON object; `{i}` in every string value becomes the shard index) is the fan-out path: the server builds one lease per shard (id `<id>-<i>`; for orchestrator kinds the token field must contain `{i}`), asks `FleetManager.plan(shards, baseline_mib, policy)` for the honest number, and answers 400 with the reason when the plan is rejected, when the shards exceed the concurrency limit ("launch in waves"), or when the worst-case cost is above the policy's approval threshold and `approve` is not set. Otherwise the job logs the plan sentence first, the leases and rendered tasks, and the RunMicrovm request for shard 0; in dry run it returns that request plus the plan; live it calls `FleetManager.lease_many`, waits for every VM to be RUNNING, and follows the fleet job until all shards are done or lost. The policy ceilings come from `MVM_LEASE_MAX_CONCURRENCY`, `MVM_LEASE_MAX_VM_SECONDS` and `MVM_LEASE_APPROVAL_USD`, each overridable per request (`max_concurrency`, `max_vm_seconds`, `approval_usd`); `baseline_mib` defaults to the image's minimum memory.
